...
const Navbar = ({ site }: { site?: any }) => {
    ...
    return (
        <div className="fixed top-6 left-0 right-0 z-50 px-4 sm:px-6 flex justify-center pointer-events-none transition-all duration-300">

            {/* The Floating Pill Navbar - Extremely Glassy */}
            <nav className="w-full max-w-6xl bg-[#091512]/20 backdrop-blur-md border border-white/10 rounded-full px-6 py-3 shadow-[0_8px_32px_rgba(0,0,0,0.4)] pointer-events-auto flex justify-between items-center transition-all duration-300">
                ...
                <div className="hidden md:flex items-center gap-6">
                    ...
                    <div className="flex items-center gap-3">
                        {user ? (
                            ...
                            <>
                                <Link href="/portal/login" className="px-5 py-2.5 rounded-full bg-[#111a17] hover:bg-white/10 text-white border border-white/10 font-medium text-sm transition-all flex items-center gap-2 relative group overflow-hidden">
                                    <div className="absolute inset-0 bg-white/5 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                                    <svg className="w-4 h-4 text-slate-400 relative z-10" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 16l-4-4m0 0l4-4m-4 4h14m-5 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h7a3 3 0 013 3v1" /></svg>
                                    <span className="relative z-10">{t('nav_login')}</span>
                                </Link>
                                <Link href="#contact" className="px-6 py-2.5 rounded-full bg-[#309673] hover:bg-[#257d5f] text-white shadow-[0_4px_15px_rgba(48,150,115,0.2)] font-semibold text-sm transition-all transform hover:scale-105">
